...
public class ComplaintPanel extends JPanel {
    ...
    public ComplaintPanel() {
        ...
        // Complaint view section (expands to fill remaining space)
        JPanel complaintView = createComplaintView();
        wrapper.add(complaintView, BorderLayout.CENTER);

        add(new Header("Complaint Management"), BorderLayout.NORTH);
    ...
    private JPanel createComplaintView() {
        ...
        // Grid for complaint cards using BoxLayout for proper sizing
        ...
        try {
            ...
            java.util.List<Complaint> complaints = complaintService.getAllComplaints();
            
            if (complaints == null || complaints.isEmpty()) {
                JLabel emptyLabel = new JLabel("No complaints available");
                emptyLabel.setForeground(SystemStyle.MUTED_TEXT);
                grid.add(emptyLabel);
            } else {
                for (Complaint complaint : complaints) {
                    JPanel card = createComplaintCard(complaint);
            ...
            e.printStackTrace();
            JLabel errorLabel = new JLabel("Error loading complaints");
    ...
    private JPanel createComplaintCard(Complaint complaint) {
        ...
        JLabel title = new JLabel("Complaint #" + complaint.getComplaintId());
        title.setFont(SystemStyle.SUBTITLEBOLD.deriveFont(15f));
        title.setForeground(SystemStyle.TEXTCOLOR);

        JLabel location = new JLabel("Location: " + complaint.getBarangayName());
        ...
        // Bottom - Status badge
        JLabel status = new JLabel(complaint.getStatus());
